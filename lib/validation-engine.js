...
function calculateConfidence(expected, scraped) {
  ...
  // Size match (10 points) - More flexible for better results
  ...
  if (scrapedSize) {
    ...
    if (sizeDifference === 0) {
      ...
      score += 9; // Very close (90ml vs 100ml)
    } else if (sizeDifference <= 25) {
      score += 7; // Close enough (75ml vs 100ml)
    } else if (sizeDifference <= 50) {
      score += 5; // Different size but same product (50ml vs 100ml)
    } else if (sizeDifference <= 100) {
      score += 2; // Very different size but still same product line
    }
  }
...
function validateProduct(expected, scraped) {
  ...
  // Debug logging for low confidence scores
  if (confidence < 50) {
    console.log(`❌ REJECTED: ${scraped.name || scraped.title} - Score: ${confidence}%`);
    console.log(`   Expected: ${expected.brand} ${expected.line} ${expected.concentration} ${expected.size}ml`);
    console.log(`   Scraped: ${scraped.brand || '?'} ${scraped.name || scraped.title} ${scraped.size || '?'}`);
  }

  // Check if confidence meets threshold (50+ for maximum coverage)
  if (confidence < 50) {
    return {
      valid: false,
      confidence,
      reason: `Rejected: Confidence score ${confidence} below threshold of 50`,
      scraped
    };
  }
